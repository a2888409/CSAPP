...
void eval(char *cmdline) 
{
    ...
    //避免addjob和deletejob的竞争，在add之前阻塞子进程的信号
    sigemptyset(&mask);
    ...
    if(!builtin_cmd(argv)){
        if((pid = fork()) == 0){
            ...
            if(setpgid(0, 0) < 0)  //设置当前进程ID为当前进程PID
